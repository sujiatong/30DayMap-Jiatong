...
qing <- st_read("/Users/jiatong/Downloads/v2_1820_prov_pgn_utf/v2_1820_prov_pgn_utf.shp", options = "ENCODING=UTF-8")

qing_1911 <- st_read("/Users/jiatong/Downloads/v6_1911_prov_pgn_utf/v6_1911_prov_pgn_utf.shp", options = "ENCODING=UTF-8") %>% st_transform(4326)
...
final <- 
ggplot() +
  geom_sf(data = qing_filter, aes(fill = "1820 (Jiaqing 25th year)"), color = NA, alpha = 0.8, size = 0.2) +
  geom_sf(data = qing_1911, aes(fill = "1911 (Xuantong 3rd year)"), color = NA, alpha = 0.3, size = 0.2) +
  # Modern China Border with dashed line and legend
  geom_sf(data = china_modern, aes(linetype = "China Mainland (1949 - Present)"), fill = NA, color = "grey30", size = 0.6) +
  
  geom_sf_text(data = qing_1911, aes(label = Official_English_Name), size = 1.8, color = "#5e5148",  nudge_y = -0.2) +

  scale_fill_manual(
    values = c("1820 (Jiaqing 25th year)" = "#d8c295", "1911 (Xuantong 3rd year)" = "#7d5a50"),
    name = "Time Period"
  ) +
  ...
  labs(
    ...
    subtitle = "1820 (Jiaqing 25th year) vs 1911 (Xuantong 3rd year)",
  ...
  theme(
    ...
    plot.subtitle = element_text(family = "serif", size = 14, hjust = 0.5, color = "#5e5148"),
    ...
    plot.caption = element_text(hjust = 0, size = 6, color = "#5e5148", family = "serif"),
    plot.margin = margin(10, 55, 10, 25)  # Increase margins around the plot (top, right, bottom, left)
  )
